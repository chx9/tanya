...
static void readPubsubMsg(aeEventLoop *el, int fd, void* privatedata, int mask){
    ...
    thread->pubsub_buffer = sdsnewlen(buf, nread);
    processPubsubMsg();
}
...
static int sendPubsubMsgToThread(char* buf, int thread_id, int buf_len){
    proxyThread *thread = proxy.threads[thread_id];
    int fd = thread->pubsub_io[THREAD_IO_WRITE];
    int nwritten = 0;
    int n = 0;
    while(nwritten<buf_len){
        n = write(fd, buf+nwritten, buf_len); 
        nwritten += n;
    } 
    return 0;
}
                                     ...
                                     int thread_id)
{
    ...
    while (ctx->reader->len > 0) {
        ...
        if(node->flags & PUBSUB_NODE){
            proxyLogInfo("pubsub msg:%s", ctx->reader->buf);
            // redirect to other thread through pipe
            for(int i=0;i<config.num_threads;i++){
                sendPubsubMsgToThread(ctx->reader->buf, i, ctx->reader->len);
            }
            goto consume_buffer;
        } 
        if (req == NULL) {
...
static void *execPubsubThread(void *ptr){
    ...
    req->buffer = sdsnew("*2\r\n$10\r\npsubscribe\r\n$1\r\n*\r\n");
    req->command = &psubscribeCommand;
    req->node = getFirstMappedNode(psThread->cluster);
    req->node->flags |= PUBSUB_NODE;
    proxyLogInfo("node ip:%s, port:%d", req->node->ip, req->node->port);
